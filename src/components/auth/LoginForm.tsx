...
import {
    useLoginForm,
    type ClassificationLevelOption,
    type ResidencyZoneOption,
} from "@/hooks/forms/auth/use-login-form";

const RESIDENCY_OPTIONS: { value: ResidencyZoneOption; label: string }[] = [
    { value: "UK_ONLY", label: "UK Only" },
    { value: "UK_AND_EEA", label: "UK + EEA" },
    { value: "GLOBAL_RESTRICTED", label: "Global" },
];

const CLASSIFICATION_OPTIONS: { value: ClassificationLevelOption; label: string }[] = [
...
function CustomSelectField<TValue extends string>({
    id,
    label,
    value,
    options,
    ...
    const handleOAuthSignIn = async (provider: "google" | "microsoft") => {
        ...
        try {
            ...
            const callbackURL = orgSlug.length > 0
                ? `/api/auth/post-login?next=%2Fdashboard&org=${encodeURIComponent(orgSlug)}`
                : "/api/auth/post-login?next=%2Fdashboard";
            await signIn.social({
                provider,
                callbackURL,
            });
...
function InputField({ label, error, icon, className, ...rest }: InputFieldProps) {
