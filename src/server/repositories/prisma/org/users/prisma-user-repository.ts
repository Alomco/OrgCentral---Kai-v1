import type { User as PrismaUser, Prisma, Membership as PrismaMembership, Organization } from '@prisma/client';
...
export class PrismaUserRepository extends OrgScopedPrismaRepository implements IUserRepository {
  async findById(id: string): Promise<User | null> {
    ...
    if (!rec) return null;
    return mapPrismaUserToDomain(rec as PrismaUser);
  }
  ...
  async findAll(filters?: UserFilters): Promise<User[]> {
    ...
    return records.map((r) => mapPrismaUserToDomain(r as PrismaUser));
  }
  ...
  async getUser(tenantId: string, userId: string): Promise<UserData | null> {
    ...
    const domainUser = mapPrismaUserToDomain(user as PrismaUser);
  ...
  async updateUserMemberships(tenantId: string, userId: string, memberships: import('@/server/types/membership').Membership[]): Promise<void> {
  ...
  async getUsersInOrganization(tenantId: string, organizationId: string): Promise<UserData[]> {
    ...
    for (const mem of memberships) {
      const user = mem.user as PrismaUser;
      const domainUser = mapPrismaUserToDomain(user);
      const domainMembership: import('@/server/types/membership').Membership = mapPrismaMembershipToDomain(mem as PrismaMembership & { org?: Organization | null });
      results.push({
        id: domainUser.id,
        email: domainUser.email,
        displayName: domainUser.displayName ?? '',
