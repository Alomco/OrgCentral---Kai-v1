    ...
    async process(payload: DocumentExpiryPayload, context: RepositoryAuthorizationContext) {
        ...
        for (const profile of profiles) {
            ...
            if (payload.thresholdDays.includes(daysUntilExpiry)) {
                if (payload.dryRun) {
                    console.info(`[DryRun] Would notify user ${profile.userId} about work permit expiry in ${daysUntilExpiry} days`);
                    continue;
                }
                ...
                await notificationComposer.composeAndSend({
                    ...
                    notification: {
                        ...
                        body: `Your Work Permit (${workPermit.type ?? 'Document'}) expires in ${daysUntilExpiry} days (${expiryDate.toDateString()}). Please renew it.`,
