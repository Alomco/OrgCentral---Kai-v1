...
export function RequestImpersonationForm() {
    ...
    const mfaRequired = state.status === 'error' && state.code === 'MFA_REQUIRED';
    const [mfaOpen, setMfaOpen] = useState(false);

    return (
        <>
            <form ref={formReference} action={formAction} className="space-y-4 rounded-2xl border border-border/50 bg-card/60 p-4">
                ...
                {state.status !== 'idle' ? (
                    <Alert>
                        <AlertDescription className="flex flex-wrap items-center gap-2">
                            ...
                            {mfaRequired ? (
                                <Button type="button" variant="outline" size="sm" onClick={() => setMfaOpen(true)}>
                                    Verify MFA
                                </Button>
                            ) : null}
                        </AlertDescription>
            ...
            <Modal open={mfaOpen} onOpenChange={setMfaOpen}>
                <ModalContent
                    size="md"
                    className="shadow-none"
                    bodyClassName="gap-4 p-0"
                >
                    <ModalHeader className="border-b border-border/60">
                        <ModalTitle className="text-base">Verify MFA</ModalTitle>
                        <ModalDescription>
                    ...
                    <ModalFooter className="border-t border-border/60">
                        <ModalClose asChild>
